...
def classify_z_what(arguments, z_what, labels, n, cfg):
    ...
    relevant_labels = [int(part) for part in arguments['indices'].split(',')] if arguments['indices'] else range(labels.max() + 1)

    folder = f'{cfg.exp_name}{cfg.seed}' if cfg else f'{arguments["folder"]}1'
    if arguments['type'] == "show":
        ...
        if arguments['method'] == "linear":
            for clf in [LogisticRegression(), LinearRegression(), SGDClassifier()]:
                for train_portion in range(500, 4501, 1000):
                    nb_sample = int(train_portion / 5000 * len(labels))
                    clf.fit(z_what[:nb_sample], labels[:nb_sample])
                    print("-" * 15)
                    print(f"classifier: {clf.__class__.__name__} nb samples: {nb_sample}")
                    acc = clf.score(z_what[nb_sample:], labels[nb_sample:])
