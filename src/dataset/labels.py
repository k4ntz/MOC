...
label_list_space_invaders = [f"score_{side}_{idx}" for side in ['left', 'right'] for idx in range(4)] + [f"enemy_{idx}" for idx in range(6)] \
                            + ["target_spaceship", "player", "blocks", "bullet", "no_label"]
...
def filter_relevant_boxes(game, boxes_batch, boxes_gt):
    if "MsPacman" in game:
        ...
        return [box_bat[box_bat[:, 1] > 19 / 128] if len(box_gt[box_gt[:, 1] < 19 / 128]) <= 1 else box_bat for box_bat, box_gt in zip(boxes_batch, boxes_gt)]
    elif "Pong" in game:
...
def get_labels(row, game, boxes_batch):
    ...
    if "MsPacman" in game:
        ...
        return _get_labels_space_invaders(row, boxes_batch)
    else:
        raise ValueError(f"Game {game} could not be found in labels")
...
def get_labels_moving(row, game, boxes_batch):
    ...
    if "MsPacman" in game:
        ...
        return _get_labels_space_invaders_moving(row, boxes_batch)
    else:
        raise ValueError(f"Game {game} could not be found in labels")
...
def _get_labels_pong_moving(row, boxes_batch):
    ...
    return labels_for_batch(boxes_batch, entity_list, row, label_list_space_invaders)


def _get_labels_space_invaders_moving(row, boxes_batch):
    entity_list = [label for label in label_list_space_invaders if "score" not in label and "no_label" not in label]
    return labels_for_batch(boxes_batch, entity_list, row, label_list_pong)

def _get_labels_space_invaders(row, boxes_batch):
    entity_list = [label for label in label_list_space_invaders if "no_label" not in label]
    return labels_for_batch(boxes_batch, entity_list, row, label_list_space_invaders)
